...
variable "filter_cluster_names" {
  type        = list(string)
  description = "List of ECS cluster names to monitor. Note that the clusters being monitored must have Container Insights enabled."
  default     = []
}
...
variable "task_count_threshold_duration" {
  type        = number
  description = "See newrelic_nrql_alert_condition.critical.threshold_duration."
  default     = 600
}
