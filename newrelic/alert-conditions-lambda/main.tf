locals {
  ...
  function_environments_quoted   = join(", ", formatlist("'%s'", var.filter_function_environments))
  function_environments_subquery = length(var.filter_function_environments) == 0 ? "" : "`tags.environment` IN (${local.function_environments_quoted})"
  ...
  filter_subqueries_and = join(" AND ", compact([local.aws_accounts_subquery, local.function_names_subquery, local.function_environments_subquery, local.function_names_exclude_subquery]))
