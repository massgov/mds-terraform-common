locals {
  aws_accounts_quoted = join(", ", [
    for account in var.filter_aws_accounts : "'${replace(account, "'", "\\'")}'"
  ])

  aws_accounts_subquery = length(var.filter_aws_accounts) == 0 ? "" : "`aws.accountId` IN (${local.aws_accounts_quoted})"

  distribution_names_quoted = join(", ", [
    for name in var.filter_distribution_names : "'${replace(name, "'", "\\'")}'"
  ])

  distribution_names_subquery = length(var.filter_distribution_names) == 0 ? "" : "`entity.name` IN (${local.distribution_names_quoted})"

  filter_subqueries_and = join(" AND ", compact([
    local.aws_accounts_subquery,
    local.distribution_names_subquery
  ]))

  filter_subquery = local.filter_subqueries_and == "" ? "" : "WHERE (${local.filter_subqueries_and})"
...
module "throughput" {
  ...
  name = format(
    "%s - Less than %d requests per %d seconds for over %d seconds",
    var.name_prefix,
