  ...
  task_containers = var.ecs_task_def != null ? [
    for t in var.ecs_task_def.containers :
    {
      ...
      secrets : [
        ...
        {
          name : s.name,
          valueFrom : s.valueFrom != null ? (startswith(s.valueFrom, "arn:aws:secretsmanager:") ? s.valueFrom : "arn:aws:ssm:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:parameter/${s.valueFrom}") : null
        }
      ]
      logConfiguration : {
        logDriver : "awslogs",
