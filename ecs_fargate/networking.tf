...
resource "aws_lb_target_group" "alb" {
  for_each = length(var.ecs_load_balancers) != 0 ? var.ecs_load_balancers : {}
...
#   for_each   = var.ecs_load_balancers.alb
  for_each = length(var.ecs_load_balancers) != 0 ? var.ecs_load_balancers.alb : {}

  listener_arn = data.aws_lb_listener.selected_443[0].arn
